...
def train(epoch):
    ...
    for batch_idx, (data0, target0, imgpaths) in enumerate(train_loader):
        ...
        for lr in [1e-1]:
            ...
            for i in range(num_iter):
                ...
                print("[" + time.asctime(time.localtime(
                    time.time())) + "]" + 'Batch num:%d Iteration: %d / %d Loss : %f noise norm: %f Fooled so far: %d Max Class Prob: %0.3f' \
        ...
        if max_class_prob < 0.35:
            ...
            png_image.save(backupdir + '/' + 'fooled' + '/' + str(int(max_class_prob * 100)).zfill(3) + '_' +
                           imgpaths[0].split('/')[-1][:-4] + '_adv' + '.png')

            png_image.save(final_result_dir + '/' + imgpaths[0].split('/')[-1][:-4] + '.png')
            np.save(noise_result_dir + '/' + imgpaths[0].split('/')[-1][:-4] + '.npy',
                    model.noise.data.squeeze().cpu().numpy())
        ...
        if not fooled:
            ...
            png_save = png_save.clip(0, 255)
            png_image = Image.fromarray(png_save)
            png_image.save(backupdir + '/' + 'notfooled' + '/' + str(int(max_class_prob * 100)).zfill(3) + '_' +
                           imgpaths[0].split('/')[-1][:-4] + '_adv' + '.png')
